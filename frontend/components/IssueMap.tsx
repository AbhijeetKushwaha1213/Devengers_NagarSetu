...
const createIssueMarkerIcon = (color: string, priority: string, isCritical: boolean) => {
  const size = isCritical ? 24 : 18;
  return L.divIcon({
    className: 'issue-marker-pin',
    html: `
...
const IssueMap: React.FC<IssueMapProps> = ({ issues, onIssueSelect }) => {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      const containerId = 'mappls-issue-map-canvas';
      ...
      if (ready && MapplsClass && MapplsClass.Map) {
        try {
          ...
          console.warn('MapMyIndia native SDK init failed for IssueMap, fallback to Leaflet:', sdkErr);
        }
      }
      ...
      try {
        ...
        console.error('Error initializing IssueMap:', err);
  ...
  useEffect(() => {
    ...
    issues.forEach((issue) => {
      ...
      const isCritical = issue.status !== 'resolved' && priority === 'critical';

      const marker = L.marker([coords.lat, coords.lng], {
        icon: createIssueMarkerIcon(color, priority, isCritical),
        title: issue.title,
      });
      ...
      const popupHtml = `
        <div style="max-width: 260px; font-family: sans-serif; padding: 4px;">
          ...
          <div style="display: flex; gap: 4px; flex-wrap: wrap; margin-bottom: 6px;">
            ...
            <span style="background: ${issue.status === 'resolved' ? '#10b981' : issue.status === 'in-progress' ? '#f59e0b' : '#64748b'}; color: white; padding: 2px 6px; border-radius: 9999px; font-size: 10px;">
  ...
  return (
    <div className="relative rounded-lg overflow-hidden border border-slate-200 shadow-sm">
      ...
      <div id="mappls-issue-map-canvas" className="w-full h-96 bg-slate-100" />
      ...
      <div className="absolute bottom-3 left-3 bg-white/95 backdrop-blur-md p-2.5 rounded-lg shadow-md border border-slate-200 z-[1000] text-xs">
        <h4 className="font-semibold text-slate-800 mb-1.5 text-[11px]">Priority Legend</h4>
      ...
      {/* Header Badge */}
      <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-lg shadow-md border border-slate-200 z-[1000] flex items-center gap-1.5 text-xs font-semibold text-slate-800">
        <MapPin className="h-3.5 w-3.5 text-blue-600" />
        <span>{issues.length} Issues Mapped</span>
      </div>
    </div>
  );
};
